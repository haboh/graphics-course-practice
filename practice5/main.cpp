...
void main()
{
    ...
    texcoord = texcoord0 + vec2(cos(time), sin(time));
}
)";
...
void main()
{
    ...
    vec3 albedo = texture(cow_texture, texcoord).xyz;
...
int main() try
{
  ...
  GLuint texture_location = glGetUniformLocation(program, "cow_texture");
  ...
  for (size_t i = 0; i < size * size; i++) {
    pixels[i] = i & 1 ? 0xFF000000u : 0xFFFFFFFFu;
  }
  ...
  for (int level = 1; level <= 3; level++) {
    int factor = 1 << level;
    pixels.clear();
    pixels.resize(size / factor * size / factor);
    ...
    glTexImage2D(GL_TEXTURE_2D, level, GL_RGBA8, size / factor, size / factor, 0, GL_RGBA, GL_UNSIGNED_BYTE, pixels.data());
  }
  ...
  stbi_uc* cow_pixels = stbi_load(cow_texture_path.c_str(), &cow_texture_width, &cow_texture_height, &cow_texture_channels, 4);
