...
void main()
{
    ...
    vec3 color = ambient;
    vec3 sun = (diffuse(sun_direction) + specular(sun_direction)) * sun_color;
    
    float light_dist = distance(position, point_light_position);
    vec3 light_dir = (position - point_light_position) / light_dist;
    vec3 light = (diffuse(light_dir) + specular(light_dir)) * point_light_color;

    out_color = vec4(color + sun + light, 0.5);
}
)";
...
int main() try {
    ...
    while (running) {
        ...
        light_position[0] = 10 * cos(time);
        light_position[1] = 10 * sin(time);
        light_position[2] = 19 * sin(time) * cos(time);

        glUniform3f(point_light_position_location, light_position[0], light_position[1], light_position[2]);
        glUniform3f(point_light_color_location, 0.5f, 0.8f, 0.8f);
        glUniform3f(point_light_attenuation_location, 1.0f, 0.0f, 0.1f);
        ...
        for (int i = 0; i < 3; i++) {
          for (int j = 0; j < 3; j++) {
            model = glm::translate(glm::mat4(1.0f), glm::vec3(scale * (j - 1), scale * (i - 1), distance));

            glUniformMatrix4fv(model_location, 1, GL_FALSE, (float*)(&model));

            float k = (i * 3 + j) / 9.0;

            glUniform1f(roughness_location, 0.1f * (1.f - k) + 0.5 * k);
            glUniform1f(glossiness_location, 1.f * (1.f - k) + 5.f * k);
